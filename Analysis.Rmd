...
params:
  input1: "data/input_bedmethyl_files/HCT116_WT_tidy.dssin.gz" # Reference group 
  input2: "data/input_bedmethyl_files/HCT116_3BKO_tidy.dssin.gz" # Test group
---

Most of the initial testing with DMR and bedMethyl files were performed in this file including the parameter comparison of DSS with different delta and p thresholds. The main dss scripts that generate the DSS output files are `dss_analysis.R` or `dss_analysis.Rmd`.
...
results <- read.csv("analysis_results/threshold_dmldmr_count_table.csv", header = TRUE)
```
...
ggplot(data = results, aes(x = -log10(pval), y = log10(DMLs), group = delta)) +
  ...
  geom_point(aes(color = delta), size = 2.6) +
...
dmrs_strict <- callDMR(dmlTest.sm,
  ...
  pct.sig = 0.5
) # % of significant CpGs required (default: 0.5)
head(dmrs_strict)
