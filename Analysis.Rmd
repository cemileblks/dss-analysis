...
## Lets make some loops

```{r}
# Parameter grid
# params <- data.frame(
#   p = c(0.05, 0.01, 0.01, 0.01, 0.005, 0.001, 0.001, 0.001, 0.0005),
#   delta = c(0.1, 0, 0.05, 0.1, 0.1, 0, 0.1, 0.2, 0.2)
# )
# 
params2 <- data.frame(
  p = c(0.05, 0.05, 0.05, 0.05, 0.01, 0.005, 0.005, 0.005, 0.001),
  delta = c(0, 0.1, 0.2, 0.05, 0.2, 0, 0.2, 0.05, 0.05)
)

params3 <- data.frame(
  p = c(0.0005, 0.0005, 0.0005),
  delta = c(0, 0.1, 0.05)
)
...
# Add a list to store DMR objects
# dmr_list <- list()

for (i in 1:nrow(params3)) {
  pval <- params3$p[i]
  delta <- params3$delta[i]
...
# Extract FDRs and convert to percentage
fdr_list <- lapply(dml_list, function(df) 100 * df$fdr)

# Combine to long data frame
fdr_df <- melt(fdr_list)
colnames(fdr_df) <- c("FDR_percent", "Threshold")

# Optional: make threshold a factor with custom order
fdr_df$Threshold <- factor(fdr_df$Threshold, levels = names(dml_list))

# Plot
ggplot(fdr_df, aes(x = Threshold, y = FDR_percent)) +
  geom_boxplot(fill = "lightblue") +
  labs(
    title = "FDR (%) Distribution per DML Detection Threshold",
    x = "Threshold setting (p + delta)",
    y = "FDR (%)"
  ) +
  theme_minimal() +
...
# Total number of CpG tests (from your BSseq object)
total_cpgs <- nrow(dmlTest.sm)  # e.g., 27145793

# Create new columns to store estimated FP and FDR
...
```
# FDR for pvalue thresholds of DMLs
