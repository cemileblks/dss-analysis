...
ordered_labels <- label_map[feature_order]
fisher_results_hyper$FeatureLabel <- factor(fisher_results_hyper$FeatureLabel, levels = ordered_labels)
fisher_results_hypo$FeatureLabel  <- factor(fisher_results_hypo$FeatureLabel, levels = ordered_labels)
...
ggplot(fisher_results_hyper, aes(x = FeatureLabel, y = OddsRatio, fill = Feature)) +
  geom_bar(stat = "identity", width = 0.8, lwd = 0.4, color = "black") +
  geom_text(aes(label = p_display), vjust = -0.5, size = 3.5) +
  coord_cartesian(ylim = c(0, 2.1)) +
  geom_hline(yintercept = 1, linetype = "dashed", color = "gray40") +
  scale_fill_manual(values = custom_colors) +
  labs(
    title = "Fold Enrichment of Hyper-DMLs by Genomic Feature",
    x = "Genomic Feature",
    y = "Fold Enrichment"
  ) +
...
ggsave("analysis_results/Fold_enrichment_hyper_dmls_in_features.png")


ggplot(fisher_results_hypo, aes(x = FeatureLabel, y = OddsRatio, fill = Feature)) +
  geom_bar(stat = "identity", width = 0.8, lwd = 0.4, color = "black") +
  geom_text(aes(label = p_display), vjust = -0.5, size = 3.5) +
  coord_cartesian(ylim = c(0, 2.1)) +
  geom_hline(yintercept = 1, linetype = "dashed", color = "gray40") +
  scale_fill_manual(values = custom_colors) +
  labs(
    title = "Fold Enrichment of Hypo-DMLs by Genomic Feature",
    x = "Genomic Feature",
    y = "Fold Enrichment"
  ) +
  theme_classic(base_size = 11) +
...
ggsave("analysis_results/Fold_enrichment_hypo_dmls_in_features.png")
